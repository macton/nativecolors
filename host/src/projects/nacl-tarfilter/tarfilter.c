...
main(int argc, char **argv)
{
	...
	if (archive_read_support_format_all(ina) != ARCHIVE_OK)
        ...
        int open_err;
	// if (open_err=archive_read_open_file(ina, argv[1], 10240) != ARCHIVE_OK)
		// die("Couldn't open input archive %d (EINVAL=%d;EFTYPE=%d)",open_err,EINVAL,EFTYPE);

	if ((open_err = archive_read_open_file(ina, argv[1], 10240))) {
		printf(archive_error_string(ina));
		printf("\n");
		exit(open_err);
	}
	...
	while ((r = archive_read_next_header(ina, &entry)) == ARCHIVE_OK) {
		fprintf(stdout, "%s: ", archive_entry_pathname(entry));
